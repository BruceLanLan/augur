...
**评审对象**：`origin/main` @ `df60456`（v11.0.0-rc1，2026-08-11～08-13 集中开发，144 个 commit、+42k 行）
...
4. **高速批量开发要配一个「真跑一遍」的关卡。** 这次所有假象（假绿 CI、导入不存在符号的测试、0→2545 的 lint）都是「本地说过了」但没人从零安装再验证一次。ROADMAP 步骤 0 把这个关卡写死。
