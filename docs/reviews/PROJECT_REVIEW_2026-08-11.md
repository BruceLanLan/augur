...
1. 测试和运行状态不够可复现，能误导入旧 editable install，并会写真实 `~/.augur`。
...
本次第一次运行测试时，进程导入了机器上的 `<repo>` 旧 editable install，而不是当前 checkout；同时多个测试写入 `~/.augur`，在受限环境中产生 13 个失败。强制当前 `src` 并隔离状态后，这 13 个失败全部消失。
...
未来 90 天不应继续横向增加 persona、资产类别或大页面。最有效的产品楔子是：
...
- 离线 pytest：
  - 非隔离环境：13 failed / 2460 passed，确认由旧 editable import 与用户目录写入引起。
