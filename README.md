...
# WeChat/微信 Bot
augur wechat                   # 个人微信模式 (推荐, GeWeChat扫码)
augur wechat --mode wecom      # 企业微信模式 (接收+发送)
...
Augur 支持通过微信进行实时分析，支持三种模式:
1. **个人微信 (GeWeChat)** - 推荐! 扫码登录个人号, Hermes Agent 同款方案
2. **企业微信 (WeCom)** - 企业场景, 接收+发送
3. **Webhook (仅推送)** - 最简单, 配合 Cron 定时推送

### 方案一: 个人微信 (GeWeChat) - 推荐

GeWeChat 是 Docker 容器化的微信协议服务, 扫码登录后即可通过 API 收发消息。
...
# 或手动启动
docker run -d --name gewechat \
  -p 2531:2531 -p 2532:2532 \
  -v gewe-data:/root/temp \
  registry.cn-chengdu.aliyuncs.com/gewechat/gewechat:latest
```
...
# 设置 Token (GeWeChat 管理界面获取)
export GEWECHAT_TOKEN='your_token'
export GEWECHAT_BASE_URL='http://127.0.0.1:2531/v2/api'  # 默认值

# 启动 (首次会显示二维码, 用微信扫码登录)
augur wechat --mode personal --port 8066
```

首次登录后 App ID 自动保存到 `~/.augur/wechat.yaml`, 下次启动无需重新扫码。

#### 可用命令 (个人微信聊天)

| 命令 | 说明 | 示例 |
|------|------|------|
| `TICKER` | 直接输入代码即分析 | `AAPL` |
| `分析 TICKER [指标]` | 17位大师共识分析 | `分析 NVDA pe=45 roe=0.85` |
| `@巴菲特 TICKER` | 向特定大师提问 | `@段永平 TSLA` |
...
最简单的模式, 仅支持单向推送到企业微信群, 适合配合 Cron 定时推送。
...
# === WeChat/微信 Bot (v5.3 个人微信 + v4.6 企业微信) ===
# 个人微信 (推荐, GeWeChat 扫码即用)
export GEWECHAT_TOKEN=your_token
augur wechat --mode personal   # 首次显示二维码, 扫码登录
...
export WECHAT_TOKEN=your_token
export WECHAT_AES_KEY=your_aes_key
augur wechat --mode wecom      # 企业微信模式
augur wechat --mode webhook    # Webhook 推送模式
